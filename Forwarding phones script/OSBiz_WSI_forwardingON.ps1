<# Bypass one-time-run: powershell.exe -noprofile -executionpolicy bypass -file .\OSBiz_WSI_forwardingON.ps1 forks.csv
...
foreach ($line in (import-csv $csvpath)) {
	...
	$cmdForkON = "$OSBizURL/gadgetapi?cmd=MkCall&callingDevice=$User&calledDirectoryNumber=*83$extension*11$forkNum&gsSession=$SessionID"
	Invoke-WebRequest -Uri $cmdForkON | Select-Object -ExpandProperty content | Out-File WSI_fork.xml
